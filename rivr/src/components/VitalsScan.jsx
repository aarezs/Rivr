...
import { Camera, Heart, Wind, Brain, CheckCircle, AlertCircle } from 'lucide-react';
    ...
    <div className="min-h-[100dvh] flex flex-col relative overflow-hidden bg-dark-bg">
      ...
      <div className="text-center pt-8 pb-4 px-4 relative z-10">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 border border-primary/20 mb-3">
          ...
          <span className="text-primary text-sm font-medium">{t('vitals.title')}</span>
        </div>
        <p className="text-text-light text-sm max-w-xs mx-auto">
          {phase === 'complete' ? t('vitals.complete') : t('vitals.instruction')}
        </p>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 relative z-10">
        {phase !== 'complete' ? (
          <div className="relative w-full max-w-sm aspect-[3/4] rounded-3xl overflow-hidden glass border-white/10 shadow-2xl">
            {/* Camera feed */}
            <video
              ref={videoRef}
              autoPlay
            ...
            {phase === 'scanning' && (
              <>
                ...
                {/* Scan line animation */}
                <div className="absolute left-8 right-8 h-0.5 bg-gradient-to-r from-transparent via-primary to-transparent opacity-60"
                  style={{ animation: 'scan-line 3s ease-in-out infinite', position: 'absolute' }} />

                {/* Progress ring overlay */}
                <div className="absolute bottom-6 left-1/2 -translate-x-1/2">
                  <div className="relative">
                    <svg className="w-28 h-28 -rotate-90" viewBox="0 0 120 120">
                      <circle cx="60" cy="60" r="54" fill="rgba(26,35,50,0.7)" stroke="rgba(255,255,255,0.1)" strokeWidth="4" />
                      <circle cx="60" cy="60" r="54" fill="none" stroke="#0ABAB5" strokeWidth="4" strokeLinecap="round"
            ...
            {cameraError && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-dark-bg-lighter">
                ...
                <p className="text-text-light text-sm text-center px-4">{t('vitals.cameraError')}</p>
                <p className="text-text-light text-xs mt-2">Using simulated scan...</p>
              </div>
            )}
          </div>
        ) : (
          ...
          <div className="w-full max-w-sm animate-fade-in-up">
            <div className="flex items-center justify-center gap-2 mb-6">
              ...
              <span className="text-success font-semibold text-lg">{t('vitals.complete')}</span>
            </div>
      ...
      {/* Continue button */}
      {phase === 'complete' && (
        <div className="px-4 pb-8 relative z-10 animate-fade-in" style={{ animationDelay: '0.5s' }}>
          <button
